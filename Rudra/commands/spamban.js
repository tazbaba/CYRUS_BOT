 const num = 9999999999 //number of times spam gets banned -1, for example 5 times 6 times will get banned
const timee = 2 // During `timee` spam `num` times will be banned
...
module.exports.handleEvent = async function ({ Users, Threads, api, event})  {
  ...
  if ((global.client.autoban[senderID].timeStart + (timee*1000)) <= Date.now()) {
    ...
    if (global.client.autoban[senderID].number >= num) {
      ...
      api.sendMessage("😻Sorry sir/ma'am Your spamming as a result you are banned\n😻ID: " + senderID + " \n😻Name: " + dataUser.name + `\n😻Reason: spam bot ${num} time/${timee}s\n\n✔️Reported to admin bot`, threadID,
    () => {
